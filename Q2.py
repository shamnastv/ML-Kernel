...
def svm(C, data, label, error=1e-6,
...
def main():
    ...
    ##Question B
    C = 20
    w1, w2, b = svm(C, train_data, train_label, view_progress=False, view_graph=True)
    ...
    C = 1e-7
    w1, w2, b = svm(C, train_data, train_label, view_progress=False, view_graph=False,
                    box_plot=False)
    print("w1,w2,b=", w1, w2, b)
    test(test_data, test_label, w1, w2, b)
    ...
    C = 0.8
    w1, w2, b = svm(C, train_data_d, train_label_d, view_progress=False,
    ...
    for i in range(50):
        ...
        for al, sv_yl, svl in zip(a, sv_y, sv):
            s += al * sv_yl * gaussian_kernal(test_data[i], svl, sigma)
        y_predict[i] = s
...
def SVM_dual(C, train_data, train_label, test_data, test_label, sigma=1):
    ...
    for i in range(100):
        for j in range(100):
            K[i][j] = gaussian_kernal(train_data[i], train_data[j], sigma)
    ...
    w = np.dot((train_label_1 * alphas).T, train_data).reshape(-1, 1)
    S = (alphas > 1e-4).flatten()
    b = np.mean(train_label_1[S] - np.dot(train_data[S], w))
    a = np.ravel(sol['x'])
    ...
    for i in range(100):
        s = 0
        for al, sv_yl, svl in zip(a, sv_y, sv):
            s += al * sv_yl * gaussian_kernal(train_data[i], svl, sigma)
        y_predict[i] = s
    y_predict = y_predict + b
    correct_label = 0
    classes = []
